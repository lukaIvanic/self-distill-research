...
def get_next_batch(config, train_iter, train_dataloader, step_num, device):
    try:
    ...
    input_ids = batch['input_ids'].to(device,
                                      non_blocking=True if config.datasetConfig.pin_memory_dataloader and device.type == "cuda" else False)
...
def get_dataloader(config, split,
                   # tokenizer_path, ctx_len, dataset_name, dataset_config, cache_dir,
    ...
    generator = torch.Generator().manual_seed(config.trainingConfig.seed)
